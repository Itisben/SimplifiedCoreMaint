...
gm_graph::gm_graph() {
    ...
    e_idx2id = NULL; // do we need this?
    e_id2idx = NULL; // do we need this?
    e_rev2idx = NULL;
    e_idx2idx = NULL;

    e_undir_idx = NULL; // for undirected graph
...
void gm_graph::make_reverse_edges() {
    ...
    e_rev2idx = new edge_t[num_edges()];
...
void gm_graph::delete_frozen_graph() {
    ...
    delete [] e_undir_idx; e_undir_idx = NULL;
...
#define LINEAR_SEARCH_THRESHOLD    16
    if ((end_edge - begin_edge) <= LINEAR_SEARCH_THRESHOLD) {
        for(edge_t e = begin_edge; e <= end_edge; e++) {
            if (node_idx[e] == to) return e;
        }
        return NIL_EDGE;
    }
    ...
    while (begin_edge < end_edge) {
        ...
        edge_t mid_edge = (begin_edge/2 + end_edge/2);
        if (((begin_edge%2)==1)&&((end_edge%2)==1)) mid_edge++;
